...
import { convertXlsxToPdf, downloadPdfFromXlsx } from '../services/xlsxToPdf'
...
function XlsxToPdfPage() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const newName = file.name.replace(/\.xlsx$/i, '.pdf')
      downloadPdfFromXlsx(blob, newName)
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-10">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">XLSX → PDF</h1>
        <p className="text-gray-500 text-sm">Converta sua planilha Excel para PDF.</p>
        <p className="text-xs text-yellow-500 mt-2">⚠️ Funciona melhor em planilhas simples.</p>
      </div>
      <div
        onClick={handleClick}
        onDrop={handleDrop}
        onDragOver={(e) => e.preventDefault()}
        className="border-2 border-dashed border-gray-700 rounded-2xl p-12 text-center cursor-pointer hover:border-green-300 transition-colors mb-6"
      >
        {loading ? (
          <div>
