...
import { convertXlsxToOds, downloadOdsFile } from '../services/xlsxToOds'

function XlsxToOdsPage() {
  ...
  async function handleFile(file: File) {
    setLoading(true)
    try {
      const blob = await convertXlsxToOds(file)
      const newName = file.name.replace(/\.xlsx$/i, '.ods')
      downloadOdsFile(blob, newName)
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-10">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">XLSX → ODS</h1>
        <p className="text-gray-500 text-sm">Converta sua planilha Excel para LibreOffice.</p>
      </div>
      <div
        onClick={handleClick}
        onDrop={handleDrop}
      ...
      >
        {loading ? (
          ...
          <>
            <div className="text-5xl mb-4">📂</div>
            <p className="text-lg font-semibold text-gray-200 mb-1">Arraste seu XLSX aqui</p>
            <p className="text-sm text-gray-500">ou <span className="text-green-400">clique para selecionar</span></p>
          </>
        )}
      </div>
