...
import { convertPdfToXlsx, downloadXlsx } from '../services/pdfToXlsx'

function PdfToXlsxPage() {
  ...
  async function handleFile(file: File) {
    setLoading(true)
    try {
      const blob = await convertPdfToXlsx(file)
      const newName = file.name.replace(/\.pdf$/i, '.xlsx')
      downloadXlsx(blob, newName)
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-10">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">PDF → XLSX</h1>
        <p className="text-gray-500 text-sm">O texto será extraído e organizado em planilha Excel.</p>
        <p className="text-xs text-yellow-500 mt-2">⚠️ Funciona melhor em PDFs com tabelas simples.</p>
      </div>
      <div
        onClick={handleClick}
        onDrop={handleDrop}
