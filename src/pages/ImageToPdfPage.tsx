...
import { convertImageToPdf, downloadPdfFromImage } from '../services/imageToPdf'

interface Props {
  type: 'jpg-pdf' | 'png-pdf'
}

function ImageToPdfPage({ type }: Props) {
  const navigate = useNavigate()
  const [loading, setLoading] = useState(false)
  const [convertedFiles, setConvertedFiles] = useState<string[]>([])

  const config = {
    'jpg-pdf': { label: 'JPG → PDF', accept: '.jpg,.jpeg', color: 'hover:border-yellow-400', ext: /\.(jpg|jpeg)$/i },
    'png-pdf': { label: 'PNG → PDF', accept: '.png', color: 'hover:border-blue-400', ext: /\.png$/i },
  }
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const newName = file.name.replace(conf.ext, '.pdf')
      downloadPdfFromImage(blob, newName)
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-gray-500 text-sm">Converta sua imagem para PDF.</p>
      </div>
      <div
        onClick={handleClick}
        onDrop={handleDrop}
      ...
      >
        {loading ? (
          ...
          <>
            ...
            <p className="text-lg font-semibold text-gray-200 mb-1">Arraste sua imagem aqui</p>
